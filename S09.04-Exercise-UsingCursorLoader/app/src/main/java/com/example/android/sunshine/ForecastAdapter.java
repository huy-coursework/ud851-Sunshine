...
class ForecastAdapter extends RecyclerView.Adapter<ForecastAdapter.ForecastAdapterViewHolder> {
    ...
    public void onBindViewHolder(ForecastAdapterViewHolder forecastAdapterViewHolder, int position) {
        ...
        if (mCursor != null && mCursor.moveToPosition(position)) {
            ...
            int weatherId = mCursor.getInt(
                    mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
            double maxTemp = mCursor.getDouble(
                    mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
            double minTemp = mCursor.getDouble(
                    mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
            String weatherForThisDay = String.format(
                    "%s - %s - %s",
